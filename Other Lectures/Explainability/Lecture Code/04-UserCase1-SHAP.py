...
#%%--------------------------------------------------------------------------
explainer_lr = shap.Explainer(predict_proba_text_lr,masker=shap.maskers.Text("word"),output_names=["Negative", "Positive"])
shap_values_lr = explainer_lr([test_text])
explainer_rf = shap.Explainer(predict_proba_text_rf,masker=shap.maskers.Text("word"),output_names=["Negative", "Positive"])
shap_values_rf = explainer_rf([test_text])
explainer_xgb = shap.Explainer(predict_proba_text_xgb,masker=shap.maskers.Text("word"),output_names=["Negative", "Positive"])
...
indices_sorted_lr = sorted(range(len(scores_lr)),key=lambda i: abs(scores_lr[i]),reverse=True)
...
indices_sorted_rf = sorted(range(len(scores_rf)),key=lambda i: abs(scores_rf[i]),reverse=True)
...
indices_sorted_xgb = sorted(range(len(scores_xgb)),key=lambda i: abs(scores_xgb[i]),reverse=True)
